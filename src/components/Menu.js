...
const Menu = () => {
  ...
  const { title, subtitle, modelImg, menuItems, alt, btnText } = menuData;

  return (
    <section id="Menu" className="min-h-[780px]">
      {/* background */}
      <div className="h-[800px] bg-menu absolute w-full max-w-[1800px] -z-0"></div>
      {/* text */}
      <div className="relative z-10 top-44">
        ...
        >
          ...
          <motion.div variants={fadeIn("down", "tween", 0.6, 1.6)}>
            <img src={modelImg} alt="meal from our wide menu" />
          </motion.div>
        </motion.div>
      </div>
      {/* menu gallery */}
      ...
      >
        <div className="container mx-auto">
          ...
          <div className="btn cursor-not-allowed w-auto max-w-[250px] mx-auto capitalize opacity-40">
            {btnText}
          </div>
        </div>
      </motion.div>
